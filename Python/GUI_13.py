...
class ArduinoControl(tk.Tk):
    ...
    def __init__(self):
        ...
        while True:
            try:
                ...
                self.result = tk.messagebox.showerror("Error", "Could not open serial port. Please check if the arduino is connected and that the correct port is selected")
                if self.result == 'ok':
                    self.close()
                    break
        ...
        for port in self.ports:
            self.com_menu.add_radiobutton(
                label=port.device, command=lambda: self.on_com_select(port.device))
    ...
    def connect_to_arduino(self):
        try:
            ...
            tk.Label(self.error_window, text="Could not open serial port. Please check if the arduino is connected and that the correct port is selected.").pack()
            retry_button = tk.Button(self.error_window, text="Retry", command=self.connect_to_arduino)
            retry_button.pack()
            return
