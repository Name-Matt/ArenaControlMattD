...
class ArduinoControl(tk.Tk): 
    def __init__(self):
        ...
        for i in range(5):
            button = tk.Radiobutton(self, text=str((i+1)*20) + "%", variable=self.percentage, value=str(i+1), command = self.set_percentage)
        ...
        for i in range(5):
            button = tk.Radiobutton(self, text="Mode " + str(i+1), variable=self.game_mode, value=chr(77+i), command = self.set_game_mode)
    ...
    def set_game_mode(self):
        """Set the game mode"""
        self.ser.write(self.game_mode.get().encode())
